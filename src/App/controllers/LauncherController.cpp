...
void LauncherController::setModel(LauncherModel *model) { m_model = model; }
  ...
  if (m_windowProvider) {
    connect(m_windowProvider, &WindowProvider::windowsChanged, this, [this]() {
      ...
      if (m_mode == Constants::ProviderWindow) {
        ...
        // If m_currentSetName is empty, we are likely in standalone mode
        // override
...
void LauncherController::setDmenuMode(bool enabled) { m_dmenuMode = enabled; }

void LauncherController::setDaemonMode(bool enabled) { m_daemonMode = enabled; }
...
void LauncherController::activate(int index, int flags) {
  ...
  QString primary =
      m_model->data(modelIndex, LauncherModel::PrimaryRole).toString();
  ...
  bool isTerminal =
      m_model->data(modelIndex, LauncherModel::TerminalRole).toBool() ||
      forceTerm || holdTerm;
  QString program;
  QStringList args;
...
void LauncherController::closeWindow(int index) {
  ...
  QString windowId =
      m_model->data(modelIndex, LauncherModel::IdRole).toString();
  if (!windowId.isEmpty()) {
...
void LauncherController::moveWindowToOutput(int index,
...
void LauncherController::toggle() { setVisible(!m_visible); }

void LauncherController::quit() { QCoreApplication::quit(); }

void LauncherController::loadSet(const QString &setName,
  ...
  if (!usingSet) {
    QString mode =
        modeOverride.isEmpty() ? Constants::ProviderDrun : modeOverride;
  ...
  if (usingSet) {
    ...
    if (activeSet.layout.width)
      setOverrides["window.width"] = QString::number(*activeSet.layout.width);
    if (activeSet.layout.height)
      setOverrides["window.height"] = QString::number(*activeSet.layout.height);
    if (!activeSet.layout.anchor.isEmpty())
      setOverrides["window.anchor"] = activeSet.layout.anchor;
    if (activeSet.layout.margin)
      setOverrides["window.margin"] = QString::number(*activeSet.layout.margin);
  ...
  for (const QString &providerName : activeSet.providers) {
    if (providerName == Constants::ProviderRun) {
      ...
      ProcessProvider::SortMode sort = (sortStr == "memory")
                                           ? ProcessProvider::MEMORY
                                           : ProcessProvider::CPU;
      auto items = ProcessProvider::scan(true, limit, sort, false);
      aggregatedItems.insert(aggregatedItems.end(), items.begin(), items.end());
    } else if (providerName == Constants::ProviderKill) {
      bool showSystem =
          Config::instance().getString("kill.show_system", "false") == "true";
      auto items =
          ProcessProvider::scan(false, -1, ProcessProvider::MEMORY, showSystem);
      aggregatedItems.insert(aggregatedItems.end(), items.begin(), items.end());
    } else if (providerName == Constants::ProviderSSH) {
      QString termCmd = Config::instance().getString("ssh.terminal", "");
      bool parseKnown = Config::instance().getString("ssh.parse_known_hosts",
                                                     "true") == "true";
      ...
      if (m_windowProvider) {
        ...
        aggregatedItems.insert(aggregatedItems.end(), wItems.begin(),
                               wItems.end());
      }
    }
  }
  ...
  // Filtering
  if (!activeSet.filter.include.isEmpty() ||
      !activeSet.filter.exclude.isEmpty()) {
    std::vector<LauncherItem> filtered;
    for (const auto &item : aggregatedItems) {
      bool keep = true;
      if (!activeSet.filter.exclude.isEmpty()) {
        if (FilterUtils::matches(item.primary, activeSet.filter.exclude) ||
            FilterUtils::matches(item.id, activeSet.filter.exclude))
          keep = false;
      }
      if (keep && !activeSet.filter.include.isEmpty()) {
        bool included = false;
        if (FilterUtils::matches(item.primary, activeSet.filter.include) ||
            FilterUtils::matches(item.id, activeSet.filter.include))
          included = true;
        if (!included)
          keep = false;
      }
...
void LauncherController::requestFocus() {
  if (m_mainWindow) {
    if (Config::instance().isDebug())
      qDebug() << "LauncherController: Forcefully requesting focus via "
                  "QWindow::requestActivate()";
    // m_mainWindow->show(); // CRASH FIX: Don't call show() repeatedly, it may
    // reset IME/Surface state
    m_mainWindow->raise();
