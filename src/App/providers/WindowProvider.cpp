...
WindowProvider::WindowProvider(QObject *parent)
    : QObject(parent), m_display(nullptr), m_registry(nullptr), m_seat(nullptr),
      m_manager(nullptr) {}
...
bool WindowProvider::initialize() {
  ...
  // Setup SocketNotifier to pump events
  int fd = wl_display_get_fd(m_display);
  m_socketNotifier = new QSocketNotifier(fd, QSocketNotifier::Read, this);
  connect(m_socketNotifier, &QSocketNotifier::activated, this,
          &WindowProvider::processWaylandEvents);

  return true;
}

void WindowProvider::processWaylandEvents() {
  if (m_display) {
    if (wl_display_dispatch(m_display) < 0) {
      qWarning() << "Wayland dispatch failed";
    }
  }
}
...
void WindowProvider::toggleFullscreen(const QString &handleStr) {
  ...
  if (m_windows.contains(handle)) {
    ...
    bool isFullscreen =
        info.state.contains(ZWLR_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_FULLSCREEN);
...
void WindowProvider::toggleMaximize(const QString &handleStr) {
  ...
  if (m_windows.contains(handle)) {
    const WindowInfo &info = m_windows[handle];
    bool isMaximized =
        info.state.contains(ZWLR_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MAXIMIZED);
...
void WindowProvider::toggleMinimize(const QString &handleStr) {
  ...
  if (m_windows.contains(handle)) {
    const WindowInfo &info = m_windows[handle];
    bool isMinimized =
        info.state.contains(ZWLR_FOREIGN_TOPLEVEL_HANDLE_V1_STATE_MINIMIZED);
...
void WindowProvider::moveToOutput(const QString &handleStr,
  ...
  if (targetOutput) {
    ...
    // If it wasn't fullscreen before, unset it so it just stays on the new
    // monitor
    if (!wasFullscreen) {
      zwlr_foreign_toplevel_handle_v1_unset_fullscreen(handle);
    }
...
void WindowProvider::registryGlobal(void *data, wl_registry *registry,
  ...
  if (strcmp(interface, "wl_seat") == 0) {
    ...
    static const zwlr_foreign_toplevel_manager_v1_listener manager_listener = {
        .toplevel = handleToplevel, .finished = handleFinished};
...
void WindowProvider::handleToplevel(void *data,
  ...
  static const zwlr_foreign_toplevel_handle_v1_listener toplevel_listener = {
      ...
      .state = toplevelState,
      .done = [](void *,
                 zwlr_foreign_toplevel_handle_v1 *) {}, // Empty done handler
...
// Output callbacks
void WindowProvider::outputGeometry(void *data, wl_output *output, int32_t x,
...
void WindowProvider::outputMode(void *data, wl_output *output, uint32_t flags,
...
void WindowProvider::outputDone(void *data, wl_output *output) {
  ...
  if (provider->m_outputs.contains(output)) {
    ...
    qDebug() << "Output configured:" << info.name << info.width << "x"
             << info.height;
  }
}

void WindowProvider::outputScale(void *data, wl_output *output,
                                 int32_t factor) {
  // HiDPI scale factor - we don't need to track this for window move
}

void WindowProvider::outputName(void *data, wl_output *output,
                                const char *name) {
  auto *provider = static_cast<WindowProvider *>(data);
  if (provider->m_outputs.contains(output)) {
...
void WindowProvider::outputDescription(void *data, wl_output *output,
