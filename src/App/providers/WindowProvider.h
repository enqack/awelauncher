 ...
 * @brief Interacts with the Wayland compositor to manage foreign toplevel
 * windows.
 *
 * This class uses the @c wlr-foreign-toplevel-management-unstable-v1 protocol
 * to enumerate windows, track their titles and application IDs, and perform
 * actions such as activation, closing, or moving between monitors.
 */
...
public:
   ...
   * @note This uses a set_fullscreen(output) hack as a direct move is not yet
...
private:
  ...
  static void toplevelTitle(void *data, zwlr_foreign_toplevel_handle_v1 *handle,
                            const char *title);
  static void toplevelAppId(void *data, zwlr_foreign_toplevel_handle_v1 *handle,
                            const char *appId);
  static void toplevelState(void *data, zwlr_foreign_toplevel_handle_v1 *handle,
                            wl_array *state);
  static void toplevelClosed(void *data,
                             zwlr_foreign_toplevel_handle_v1 *handle);

  // Output callbacks
  static void outputGeometry(void *data, wl_output *output, int32_t x,
