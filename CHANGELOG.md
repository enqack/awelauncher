...
### Fixed
- **CLI Overrides**: Fixed `--margin`, `--width`, and `--height` flags being ignored (overrides now correctly map to integer config values).
- **Margins**: Fixed window margin application on Layer Shell compositors by updating QML property assignment syntax.
- **Focus Border**: Added visual focus indicator (accent-colored border) for active state to mimic window manager decoration.
...
- **Provider Sets**: named configurations grouping multiple providers with filters and custom layouts (e.g., `--set dev`).
